    ...
    <div className="flex w-full flex-col items-center gap-4 lg:flex-row-reverse lg:items-start lg:justify-center">
      ...
      <div className="flex w-full justify-between gap-6 lg:w-auto lg:flex-col">
        <div className="flex flex-row gap-2 lg:flex-col">
          ...
          <div className="inline flex-col items-center justify-center rounded-xl bg-white/5 px-2 py-2 text-neon-cyan border border-neon-cyan/20 shadow-[0_0_15px_rgba(6,182,212,0.1)] backdrop-blur-md md:px-5">
            <span className="text-[10px] uppercase tracking-widest sm:text-xs">Recorde</span>
            <p className="text-base font-semibold sm:text-3xl">
              {boardStore.highscore}
            </p>
          </div>
          ...
          {[...Array(4)].map((_, rIndex) => (
            <div key={rIndex} className="flex gap-[1vw] lg:gap-[1vh]">
              {[...Array(4)].map((_, cIndex) => {
                return (
                  <div
                    className="h-[22vw] w-[22vw] rounded-[1.5vw] border border-white/5 bg-white/5 lg:h-[18vh] lg:w-[18vh] lg:rounded-[1vh]"
                    key={cIndex}
                  />
                );
              })}
      ...
      {openModal && (
        <BasicModal
          isOpen={openModal}
          title={boardState.won ? "Você venceu!" : "Fim de jogo"}
