...
export default function MemoryMatchBoard() {
  ...
  const handleSetVisible = (tile: MMH.MemoryMatchTile) => {
    if (tile.isCorrectGuessed || tile.isCurrentlyVisible) return;
  ...
  const handleGameWon = (currentCommonID: number) => {
    if (
      ...
      const newRecord = boardStore.ls === null || boardStore.cs < boardStore.ls;
  ...
  return (
    <>
            ...
            return (
              <button
                ...
                className={classNames(
                  "relative inline-flex h-full w-full select-none rounded-md border bg-white p-2 shadow-sm transition-all duration-200 hover:shadow-[0_0_15px_rgba(236,72,153,0.4)] dark:bg-[#0f0f15] md:border-2",
                  tile.isCurrentlyVisible ? `${styles.spinx}` : "",
                  tile.isCorrectGuessed
                    ? "border-pink-400 bg-pink-500/20 shadow-[0_0_15px_rgba(236,72,153,0.3)] md:border-[3px]"
                    : "border-pink-500/40",
                )}
                style={
        ...
        <div className="flex w-full justify-end lg:w-auto">
          <div className="flex w-full flex-col-reverse justify-between gap-4 sm:flex-row lg:flex-col">
            ...
            <div className="flex w-full justify-end gap-2 lg:flex-col-reverse">
              ...
              >
                <ArrowPathIcon className="h-8 w-8 stroke-zinc-900 dark:stroke-pink-400" />
              </button>
              <button
              ...
              >
                <Cog6ToothIcon className="h-8 w-8 stroke-zinc-900 dark:stroke-pink-400" />
              </button>
              <button
              ...
              >
                <InformationCircleIcon className="h-8 w-8 stroke-zinc-900 dark:stroke-pink-400" />
              </button>
              {openHowToPlayModal && (
                <HowToPlayModal
      ...
      {(isGameLost || isGameWon) && (
        <BasicModal
          title={isGameWon ? "Você venceu!" : "Você perdeu"}
          ...
          {isGameLost && (
            <div className="mt-4">
              <span className="text-sm font-normal">
                Ops! Você clicou na mesma bomba{" "}
              ...
              <span className="text-xs font-semibold text-pink-600 dark:text-pink-400">
                Dica: Lembre-se sempre da posição da bomba
              </span>
            </div>
          )}
          {isGameWon && (
            <div className="mt-4">
              <p className="mb-1 text-sm text-gray-500">
                ...
                } no quadro de tamanho ${boardStore.bs}`}
              </p>
